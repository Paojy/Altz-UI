...
G.Options = {
	...
	PlateOptions = {
		...
		{ -- 4 目标名字字体大小
			key = "targetnamefontsize",
			option_type = "slider",
			text = T.split_words(TARGET,NAME,L["字体"],L["大小"]),
			min = 5,
			max = 25,
			step = 1,
			apply = function()
				T.ApplyUFSettings({"Tag_TargetName", "Health", "Power"}, "Altz_Nameplates")
			end,
			rely = "enableplate",
		},
		{ -- 6 图标数量
		...
		{ -- 38 友方只显示名字
			key = "nameplateShowOnlyNames",
			option_type = "cvar_check",
			text = T.split_words(L["原生姓名板"],L["友方只显示名字"]),
			arg1 = "1",
			arg2 = "0",
			store = true,
			rely = "forceNameplateCVar",
		},
		{ -- 39 横向分散
			key = "nameplateOverlapH", --0.8
			option_type = "cvar_slider",
			text = L["姓名板横向分散"],
			min = .1,
			max = 1.5,
			step = 0.05,
