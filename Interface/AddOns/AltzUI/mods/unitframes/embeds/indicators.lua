...
oUF.Tags.Methods['Mlight:pom'] = function(u) -- 愈合祷言
    local _, _,_, c, _,_,_, fromwho = UnitBuff(u, GetSpellInfo(41635))
	if c then
...
oUF.Tags.Methods['Mlight:atonement'] = function(u) -- 救赎
    local _, _,_,_,_,_, expirationTime, fromwho = UnitBuff(u, GetSpellInfo(194384))
    if(fromwho == "player") then
...
oUF.Tags.Methods['Mlight:rnw'] = function(u) -- 恢复
    local _, _,_,_,_,_, expirationTime, fromwho = UnitBuff(u, GetSpellInfo(139))
    if(fromwho == "player") then
        local spellTimer = expirationTime - GetTime()
...
oUF.Tags.Methods['Mlight:tlzg'] = function(u)
	local _, _,_,_,_,_, _, fromwho = UnitBuff(u, GetSpellInfo(208065))
	if(fromwho == "player") then
		return "|cff0000FFb|r"
	end
end --图雷之光
...
oUF.Tags.Methods['Mlight:lb'] = function(u) -- 生命绽放
    local _, _,_, c,_,_, expirationTime, fromwho = UnitBuff(u, GetSpellInfo(33763))
    if(fromwho == "player") then
		local spellTimer = (expirationTime-GetTime())
...
oUF.Tags.Methods['Mlight:rejuv'] = function(u) -- 回春
    local _, _,_,_,_,_, expirationTime, fromwho = UnitBuff(u, GetSpellInfo(774))
	local _, _,_,_,_,_, expirationTime2, fromwho2 = UnitBuff(u, GetSpellInfo(155777))
	local text, text2 = "", ""
    if (fromwho == "player") then
        local spellTimer = (expirationTime-GetTime())
		local TimeLeft = T.FormatTime(spellTimer)
...
oUF.Tags.Methods['Mlight:regrow'] = function(u) -- 愈合
	local _, _,_,_,_,_, expirationTime, fromwho = UnitBuff(u, GetSpellInfo(8936))
    if(fromwho == "player") then
        local spellTimer = (expirationTime-GetTime())
		local TimeLeft = T.FormatTime(spellTimer)
...
oUF.Tags.Methods['Mlight:wildgrowth'] = function(u) -- 野性成长
	local _, _,_,_,_,_, expirationTime, fromwho = UnitBuff(u, GetSpellInfo(48438))
    if(fromwho == "player") then
        local spellTimer = (expirationTime-GetTime())
		local TimeLeft = T.FormatTime(spellTimer)
        return "|cff33FF33"..TimeLeft.."|r"
...
oUF.Tags.Methods['Mlight:snla'] = function(u) --塞纳里奥结界
	local _, _,_,_,_,_, _, fromwho = UnitBuff(u, GetSpellInfo(102351))
	if(fromwho == "player") then
...
oUF.Tags.Methods['Mlight:rip40'] = function(u) --激流40%
	if IsEquippedItem(137085) then
		local health, max_health = UnitHealth(u), UnitHealthMax(u)
...
oUF.Tags.Methods['Mlight:ripTime'] = function(u) --激流
    local _, _,_,_,_,_, expirationTime, fromwho = UnitBuff(u, GetSpellInfo(61295))
    if(fromwho == "player") then
        local spellTimer = (expirationTime-GetTime())
		local TimeLeft = T.FormatTime(spellTimer)
        if spellTimer > 0 then
...
oUF.Tags.Methods['Mlight:beacon'] = function(u) --道标
    local _, _,_,_,_,_, _, fromwho = UnitBuff(u, GetSpellInfo(53563))
	local _, _,_,_,_,_, _, fromwho2 = UnitBuff(u, GetSpellInfo(156910))
	local _, _,_,_,_,_, _, fromwho3 = UnitBuff(u, GetSpellInfo(200025))
...
oUF.Tags.Methods['Mlight:fyxy'] = function(u) -- 赋予信仰
    local _, _,_,_,_,_, expirationTime, fromwho = UnitAura(u, GetSpellInfo(223306))
    if(fromwho == "player") then
        local spellTimer = (expirationTime-GetTime())
		local TimeLeft = T.FormatTime(spellTimer)
        if spellTimer > 0 then
...
oUF.Tags.Methods['Mlight:jhzq'] = function(u) -- 精华之泉
    local _, _,_,_,_,_, expirationTime, fromwho = UnitAura(u, GetSpellInfo(191840))
    if(fromwho == "player") then
        local spellTimer = (expirationTime-GetTime())
		local TimeLeft = T.FormatTime(spellTimer)
        if spellTimer > 0 then
...
oUF.Tags.Methods['Mlight:sooth'] = function(u)-- 抚慰之雾
	local _, _,_,_,_,_, _, fromwho = UnitAura(u, GetSpellInfo(115175))
	if (fromwho == "player") then
...
oUF.Tags.Methods['Mlight:mist'] = function(u) -- 氤氲之雾
    local _, _,_,_,_,_, expirationTime, fromwho = UnitAura(u, GetSpellInfo(124682))
    if(fromwho == "player") then
        local spellTimer = (expirationTime-GetTime())
		local TimeLeft = T.FormatTime(spellTimer)
        if spellTimer > 0 then
...
oUF.Tags.Methods['Mlight:remist'] = function(u) -- 复苏之雾
    local _, _,_,_,_,_, expirationTime, fromwho = UnitAura(u, GetSpellInfo(115151))
    if(fromwho == "player") then
        local spellTimer = (expirationTime-GetTime())
		local TimeLeft = T.FormatTime(spellTimer)
        if spellTimer > 0 then
...
oUF.Tags.Methods['Mlight:ayj'] = function(u) --暗夜井释放
    local _, _,_,_,_,_, _, fromwho = UnitBuff(u, GetSpellInfo(225724))
...
oUF.Tags.Methods['Mlight:lt'] = function(u) --基尔加丹的蓝图
    local _,_,_,_,_,_, _, fromwho = UnitBuff(u, GetSpellInfo(242622)) -- hot
	local _,_,_,_,_,_, _, fromwho2 = UnitBuff(u, GetSpellInfo(242623)) -- 吸收盾
    if (fromwho == "player") then
        return "|cffFF3030b|r"
...
oUF.Tags.Methods['Mlight:da'] = function(u) --信仰档案
    local _,_,_,_,_,_, _, fromwho = UnitBuff(u, GetSpellInfo(242619)) -- hot
	local _,_,_,_,_,_, _, fromwho2 = UnitBuff(u, GetSpellInfo(242621)) -- 吸收盾
    if (fromwho == "player") then
        return "|cffFFD700b|r"
	elseif (fromwho2 == "player") then
