...
function SetUp(framen, ...)
	...
	frame_bags_toggle:SetPoint("TOPRIGHT", "aBag_"..framen, "TOPRIGHT", -50, -12)
	frame_bags_toggle:EnableMouse(true)
	
	local frame_bags_toggle_text = createtext(frame_bags_toggle, "OVERLAY", 12, "OUTLINE", "CENTER")
	...
	bagsort:SetPoint("LEFT", frame_bags_toggle, "RIGHT", 6, 0)
	bagsort:EnableMouse(true)
	
	local bagsortt = createtext(bagsort, "OVERLAY", 12, "OUTLINE", "CENTER")
	...
	close:SetSize(14, 14)
	close:SetPoint("LEFT", bagsort, "RIGHT", 6, 0)
	close:EnableMouse(true)
	
	local closet = createtext(close, "OVERLAY", 12, "OUTLINE", "CENTER")
	closet:SetAllPoints()
	closet:SetText("X")
...
BagItemSearchBox:SetScript("OnUpdate", function()
	...
	BagItemSearchBox:SetSize(4*(config.spacing+config.size)-3, 15)
	BagItemSearchBox:SetPoint("LEFT", ContainerFrame1MoneyFrame, "RIGHT", 5, 0)
end)

BankItemSearchBox:SetScript("OnUpdate", function()
	BankItemSearchBox:ClearAllPoints()
	BankItemSearchBox:SetSize(4*(config.spacing+config.size)-3, 15)
	BankItemSearchBox:SetPoint("LEFT", ContainerFrame2MoneyFrame, "RIGHT", 5, 0)
end)
...
function ContainerFrame_GenerateFrame(frame, size, id)
	...
	for bag = 1, 5 do
		local slots = GetContainerNumSlots(bag-1)
			for item = slots, 1, -1 do
				...
				ContainerFrame1MoneyFrame:SetPoint("TOPLEFT", _G["aBag_bag"], "TOPLEFT", 6, -10)
	...
	local numrows, lastrowbutton, numbuttons, lastbutton = 0, ContainerFrame1Item1, 1, ContainerFrame1Item1
		for bank = 1, 28 do
			...
			ContainerFrame2MoneyFrame:SetPoint("TOPLEFT", _G["aBag_bank"], "TOPLEFT", 6, -10)
